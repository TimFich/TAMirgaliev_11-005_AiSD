...
public class RedBlackTree<T extends Comparable<T>> implements IRedBlackTree<T>, Iterable<T>, Iterator<T> {
    ...
    public class Node {
        ...
        public Node(T value, NodeColor color) // конструктор который задает цвет и значение
        {
            _value = value;
            _color = color;
            _parent = _nil;
        ...
        public Node getSuccessor() // возращает следующий по значению узел дерева
        {
            Node temp = null;
            Node node = this;
            if (!node.isRightFree()) {
                temp = node.getRight();
                while (!temp.isLeftFree()) temp.getLeft();
                return temp;
            }
            temp = node.getParent();
    ...
    public static <T extends Comparable<T>> void printNodes(RedBlackTree<T> tree, ArrayList<RedBlackTree<T>.Node> nodes) {
        int childesCounter = 0;
        int nodesAmount = nodes.size();
        ArrayList<RedBlackTree<T>.Node> childes = new ArrayList<RedBlackTree<T>.Node>();

        for (int i = 0; i < nodesAmount; i++) {
            if (nodes.get(i) != null && nodes.get(i) != tree._nil) {
                System.out.println("(" + nodes.get(i).getValue().toString() + "," + nodes.get(i).getColorName() + ")");
                if (!nodes.get(i).isLeftFree()) {
                    childes.add(nodes.get(i).getLeft());
                    childesCounter++;
                } else childes.add(null);
                if (!nodes.get(i).isRightFree()) {
                    childes.add(nodes.get(i).getRight());
                    childesCounter++;
                } else childes.add(null);
            } else
    ...
    public boolean contains(T value) {
        return (findNode(value) != _nil);
    }


    private Node findNode(T value) {
        Node node = _root;
        while (node != null && node != _nil && node.getValue().compareTo(value) != 0) {
